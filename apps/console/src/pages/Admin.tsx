...
export function Admin() {
    ...
    return (
      <div className="stack">
        ...
        <div className="callout callout--info">
          Admin only. Connect the address equal to <span className="mono">Config.admin</span> to see
          these controls. This is "not for you," not "broken."
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="stack">
      ...
      <div className="callout callout--info">
        Authority is the <span className="mono">x/group</span> policy: the console builds the
        message; the group process executes it. The exact message JSON is shown before signing.
      </div>
      ...
      <Panel title="Halt / resume">
        <p className="muted" style={{ marginBottom: 12 }}>
          Blast radius: stops RunEpoch, continuations, ServiceRedemptions, and purge.
        </p>
        <div className="row" style={{ gap: 8 }}>
          <button
            type="button"
            className="btn btn--danger"
            onClick={() => submitDanger("Set halted", msg.setHalted(true))}
          >
            Halt contract
          </button>
          <button
            ...
            onClick={() => submitDanger("Resume contract", msg.setHalted(false))}
          >
            Resume contract
          </button>
        </div>
      ...
      <Panel
        title="Vault pause / unpause"
        actions={
          <Pill tone={vault.data?.paused ? "serious" : "good"}>
            {vault.data?.paused ? "paused" : "active"}
          </Pill>
        }
      >
        <PauseControls
          onPause={(reason) => submitDanger("Pause vault", msg.pauseVault(reason))}
          onUnpause={() => submitDanger("Unpause vault", msg.unpauseVault())}
        />
      </Panel>
      ...
      <Panel title="Recovery">
        <p className="muted" style={{ marginBottom: 12 }}>
          ClearPendingDelegations is safe: withdrawn nhash stays in the contract; the next epoch's
          return settlement reconciles the receipt (contract §9.9).
        </p>
        <button
          type="button"
          className="btn btn--danger"
          onClick={() => submitDanger("Clear pending delegations", msg.clearPending())}
        >
  ...
  function submitConfig(fields: UpdateConfigFields) {
    tx.submit({
      ...
      tier: "danger",
      consequence:
        "Only the changed fields are submitted. Review the diff and message JSON before signing.",
  ...
  function submitDanger(title: string, message: Parameters<typeof tx.submit>[0]["message"]) {
    tx.submit({
      ...
      message,
      tier: "danger",
      consequence: "Irreversible / high-impact admin action. Type the action name to confirm.",
      onDone: () => refresh(["config", "epoch", "vault"]),
    });
  }
}
...
function ConfigEditor({
  ...
  c: ConfigResponse;
  onSubmit: (f: UpdateConfigFields) => void;
}) {
  ...
  if (Number(threshold) !== c.performance_threshold_bps)
    changed.performance_threshold_bps = Number(threshold);
  ...
  return (
    ...
    >
      <div className="grid-2">
        ...
        {bpsField(
          "Performance threshold (bps)",
          threshold,
          setThreshold,
          c.performance_threshold_bps,
        )}
      ...
      <div style={{ marginTop: 12 }}>
        {changedKeys.length === 0 ? (
          ...
          <div className="callout callout--info">
            diff:{" "}
            {changedKeys.map((k) => `${k}=${(changed as Record<string, number>)[k]}`).join(", ")}
          </div>
        )}
      </div>
      <button
    ...
    <div className="row" style={{ alignItems: "flex-end", gap: 12 }}>
      <label className="field" style={{ flex: 1 }}>
        ...
        <input
          ...
          placeholder="reason shown in the paused banner"
        />
      </label>
      <button
        type="button"
      ...
      <button type="button" className="btn btn--secondary" onClick={onUnpause}>
        Unpause vault
      </button>
    </div>
  );

...
export function TxProvider({ children }: { children: ReactNode }) {
  ...
  // §10.2 step 2: the fee is the chain's simulate result over the EXACT
  // rendered message, priced verbatim (chain-facts §flatfees) and shown
  // before anything is signed. A simulation failure surfaces the chain's
  // error verbatim and the confirm proceeds nowhere.
  useEffect(() => {
  ...
  // The confirm may proceed only once the fee is a computed fact (or in mock
  // mode, where nothing broadcasts). A failed or in-flight simulate blocks it.
      ...
      {pending && (
        ...
        <div className="sheet-overlay" role="dialog" aria-modal="true" onClick={close}>
          ...
          <div className="sheet" onClick={(e) => e.stopPropagation()}>
            ...
            {/* §17 honesty: the figure below is the chain's own simulate
                result, used verbatim (chain-facts §flatfees — the simulate
                return IS the flat per-message fee; no gas × price math
                exists to state). Gated by test/tx-fee.test.ts. */}

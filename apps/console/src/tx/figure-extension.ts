// Figure Wallet browser-extension adapter — MIRROR-TRACKED against
// apps/web/app/wallet/figure-extension.ts (the source of truth; a divergence
// found by W1's certification run is fixed in BOTH vendor modules and
// recorded in app-spec §14.1). PROVISIONAL against the same injected surface
// the App certifies: `window.figure.provenance` with connect/signDirect.
//
// `signDirect` ONLY: an extension without it gets a specific error, never a
// silent amino fallback that would sign different bytes than the ones the
// confirm sheet's decode-back gate covers.

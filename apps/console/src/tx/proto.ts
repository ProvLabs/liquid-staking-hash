// Minimal protobuf wire primitives — MIRROR-TRACKED against
// apps/web/app/tx/proto.ts (the source of truth; update both in the same
// change). Dependency-free on purpose: the console encodes four message
// shapes and decodes one (for the sign-binding gate), and a proto toolchain
// would be a larger supply-chain surface than the lines it replaces
// (SECURITY.md dependency posture; PR 8.4b §2.4 resolved the recorded
// "re-add cosmjs" constraint to the empty set).
//
// Canonical proto3 rules this encoder follows (pinned by the byte-golden
// cases in test/tx-build.test.ts):
...
// ── Reader (the sign-binding gate's decode-back path) ───────────────────
...
/** Byte equality, written once so no consumer re-implements it subtly wrong. */

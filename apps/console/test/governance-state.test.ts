// The governance panel's honesty matrix (§4b C4; chain-facts §x/group 2/3/7/
// 8/9), gated over the PURE derivation layer the page consumes. Each cell
// below is a named public-facing honesty rule under D22 — a panel that
...
describe("the zeros-tally rule (§x/group 7)", () => {
  it("an open proposal's tally comes from the Tally query, never final_tally_result", () => {
    // final_tally_result is POISONED with zeros here; the live tally has the
    // real numbers. Rendering the zeros would assert "nobody voted".
...
describe("no-group vs could-not-check (§x/group 8)", () => {
  // The fetch layer encodes the rule structurally: a 404 on the policy lookup
  // RETURNS { state: "no-group" } (a valid state the header renders as such),
  // while every other failure THROWS into the cell's error channel and
  // renders "could not check". These fixtures pin the two shapes apart so a
  // refactor collapsing them fails here.
...
describe("truncation is labeled, never a prune signal (§x/group 3, 9)", () => {
  it("the bounded shape carries the cap hit in the type", () => {
    ...
    // The page renders the truncated label off this flag; the flag reaching
    // the page type-checked is what this pin protects — removing `truncated`
    // from Bounded<T> to "simplify" breaks this test and the page together.
  });
});

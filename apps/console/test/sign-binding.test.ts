// Sign-what-you-see gate (§4 invariant 2): the bytes handed to the wallet
// decode BACK to exactly the message object and funds the confirm sheet
// displayed. Any drift between preview and signed bytes — a re-derived field,
// a mutated amount, funds appearing on a fundless action — fails the
// decode-back deep-equality here.
...
describe("decode-back equality: signed bytes == rendered message", () => {
  ...
  for (const c of cases) {
    it(c.name, () => {
      ...
      // Deep equality of the OBJECT the sheet displayed — not a string
      // comparison, so key-order in the preview cannot mask a value drift.

// The drill → notifier tick → bell chain, end to end. Every hop is the
// production implementation — no in-memory double anywhere. Run by the
// driver's `bell` phase; standalone-skippable.
...
test.describe("bell drill (CO-14)", () => {
  ...
  test("a real redemption reaches the bell through the notifier within two ticks", async ({
    ...
    // redemption_update is default-on (spec R2): no opt-in step belongs
    // here.

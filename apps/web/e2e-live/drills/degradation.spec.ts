// The master-plan degradation drills (8.1 §2.6), observation half: the
// host-side driver (infra/devnet/drills.sh) sequences the failures; these
// specs only observe HTTP. Each degraded-phase case asserts the POSITIVE
// presence of the labeled degraded state (invariant 1: a drill FAILS on
// silence), and the `baseline` phase asserts the absence of every degraded
// state first — so a later positive is caused, not ambient. Every degraded
// phase also asserts CO-23: the §14.4 coming-soon shells survive as labeled
// states, never blank and never an error page.

// Offline session fabrication (8.3 §2.2) — the e2e-live login recipe applied
// to the offline suite: POST /session/nonce → ADR-36 signature in the TEST
// PROCESS → POST /session/login → install the cookie. Everything runs against
// the server's own HTTP surface; no test-injection seam exists in `app/`
// (the standing rule), no session backdoor, no role column — role-bearing
// renders come from the (mocked) chain reads alone.
//

// The grant knob's inertness and derivation gates (8.3 §2.3, invariant 4):
// with NVHASH_MOCK_GRANT_ROLES unset, the three touched handlers answer
// BYTE-IDENTICAL to the captured fixtures — asserted against the fixture
// JSON, never against a second copy of the handler's output — so "mocks must
// not invent state" stays true for every consumer that does not opt in. With
// the knob set, each response is the fixture plus exactly one

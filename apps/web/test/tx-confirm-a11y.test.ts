// Component-level confirm-step semantics (8.3 §2.6): the offline e2e cannot
// reach the confirm end-to-end (the mock refuses to fabricate gas), so its
// keyboard/AT contract is pinned HERE — DOM order summary → disclosure →
// cancel → confirm, the disclosure a native operable <details>, the tier
// carried by TEXT never color alone — and the live lane exercises the real
// step. Rendered with react-dom/server (node env, no browser).

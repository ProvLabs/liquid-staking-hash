...
- [ ] Live MAINNET chain params: `unbonding_time`, staking `MaxEntries`
      ...
      higher limit. **Measurement path landed 2026-08-14 (PR 8.4, CO-29):**
      the testnet bootstrap's step 6 records `MsgStoreCode` gas_used with
      date/chain/height/txhash/size/sha256 and appends it here at the first
      pilot run (`infra/deploy/bootstrap/testnet-deploy.sh`;
      chain-facts §testnet row 3). Measured in Phase A deliberately: a
      cap-exceeding upload is a contract-side remedy that must land BEFORE
      the S1 audit freezes the contract — 8.5 would be too late

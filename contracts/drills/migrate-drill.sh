#!/usr/bin/env bash
# Migrate drill: unauthorized migrate rejected under a keyless policy admin;
# group proposal executes the migrate; v1 migrate transforms no state
# (byte-for-byte dump diff, contract_info excepted); cw2 gate refuses a
# downgrade live.
#

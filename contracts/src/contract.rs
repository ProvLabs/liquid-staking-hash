...
/// the admin group policy per spec §12, so a migration carries the same group
/// threshold as any admin action). Errors if the stored cw2 contract name is
/// not [`CONTRACT_NAME`] (a foreign contract's state must never be adopted)
/// or if the stored version is NEWER than this code's (a downgrade would
/// resurrect fixed flaws — the F1–F9 class); equal versions are accepted so
/// re-migration is idempotent, which is what lets the migrate drill assert
/// byte-for-byte state identity. Otherwise re-stamps the cw2 version and
/// touches NO other state — no state walk (unbounded work); a future
/// storage-layout change writes its transformation here, gated by this
/// version check, and must handle the `Releasing` epoch phase explicitly.
#[cfg_attr(not(feature = "library"), entry_point)]
...
mod unit {
    ...
    fn config_stored_before_the_margin_field_deserializes_at_50() {
        // The serde-default regression pin (8.4a §2.4): a pre-8.4a Config JSON
        // (no redemption_margin_bps key) must load as 50 — the FUNCTION
        // default, never a bare zero, or the v1 migrate would silently remove
        // the margin from existing state.
        let old_json = r#"{
            "admin": "pb1admin",

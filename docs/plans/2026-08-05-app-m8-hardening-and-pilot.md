...
| CO-25 | Console: §14.13 entity anchors not recorded in console-spec §14; governance panel + `governance` verify target; extension-wallet adapter absent (cosmjs `elliptic` constraint); stale 1905nhash gas price; CSP `connect-src` list (M4→M7 accumulation) | console-spec; `apps/console/CLAUDE.md`; m7 D8 | 8.4b (core scope) |
...
| CO-56 | **The indexer's single-writer invariant is neither enforced nor gated.** `replicas: 1` + `Recreate` bounds supervisor overlap but does not prevent it — a Deployment is at-least-one by design, so a pod deletion starts the replacement while the old container still writes, and an unreachable node is unbounded. Two writes are unguarded: `writeNav` (no `observedHeight` predicate) and the `runWindow` cursor advance (unconditional, so overlap can move a cursor backwards). No CI check prevents someone restoring `RollingUpdate`, which reads as an improvement. Options: (a) StatefulSet with `replicas: 1` — the primitive with at-most-one-per-ordinal; (b) guard both writes in SQL (`observedHeight` predicate + `GREATEST` cursor advance), which matches `services/indexer/CLAUDE.md`'s own rule and is unit-testable without Kubernetes; (c) build the rendered-chart invariant check (design §4b G2). **(b) is the only option that makes this an enforced mechanism rather than a topology assumption**, per §7's definition. Grace period bounded in the meantime. **ADR-003 sets the pattern the api/web/console legs will copy, so this wants deciding before they land** | §6 **D32** |
...
| **D30** | **8.1 scope edges**: do the three deferred reconciler kinds (`vault_paused`, `jail_report`, `epoch_overdue` — each needs a live decoder) land inside 8.1; does jail e2e-live get a dedicated-chain scheduled lane or stay a recorded gap (CO-19). | 8.1 | 8.1 plan |
...
| **D32** | **How the indexer's single-writer invariant becomes an enforced mechanism** (CO-56). Today it is bounded, not enforced: `replicas: 1` + `Recreate` + a 20 s grace period closes the rollout case, but a Deployment is at-least-one by design, so a pod deletion starts the replacement while the old container still writes and an unreachable node is unbounded. Two writes are unguarded — `writeNav` (no `observedHeight` predicate) and the `runWindow` cursor advance (unconditional, so overlap can move a cursor **backwards**) — and `runWorker` reads its checkpoint once at startup, so two supervisors stay offset. Options: **(a)** StatefulSet with `replicas: 1` (at-most-one per ordinal); **(b)** guard both writes in SQL (`observedHeight` predicate + `GREATEST` cursor advance) — the only option that satisfies §7's "mechanism plus gating check, never a topology assumption", matches `services/indexer/CLAUDE.md`'s own rule, and is unit-testable without Kubernetes; **(c)** the rendered-chart invariant check (design §4b G2), which catches a `RollingUpdate` regression but does not fix the deletion case. (b) and (c) compose. **ADR-003 is the pattern the api/web/console legs copy, so decide before they land, not after.** | the api/web/console 8.4 legs; not the indexer deploy itself | before the next 8.4 leg |
...
*2026-08-18: code review of the indexer leg added **CO-56 / D32** — the
single-writer invariant is bounded (`replicas: 1` + `Recreate` + a 20 s grace
period) but **not enforced**: a Deployment is at-least-one by design, and two
indexer writes are unguarded (`writeNav` and the `runWindow` cursor advance).
Under §7's definition that is a topology assumption, which is what §7 forbids. It
does not block the indexer deploy; it **does** want deciding before the
api/web/console legs copy the ADR-003 pattern. The review also closed a verified
credential path — a developer's gitignored `.env` entering the image through the
build context — and corrected the deploy design's §4 table, which had named an
unbuilt check as its gating mechanism.*

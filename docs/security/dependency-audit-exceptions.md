...
| RUSTSEC-2026-0258 | cargo | h2 0.3.27 | Carlton Hanna | Dev-dependency-only: reached solely through `provwasm-test-tube → cosmrs → tendermint-rpc → reqwest → hyper → h2` (`cargo tree -i` verified 2026-08-18), the embedded-chain TEST harness. Never compiled into the shipped wasm, which has no HTTP stack. The advisory is a remote memory-exhaustion DoS requiring a malicious HTTP/2 peer; the harness client only speaks to the local embedded test chain. The fix requires h2 ≥0.4.16 (hyper 1.x / reqwest 0.12), a different major incompatible with the pinned test stack, which does not move without design review (contracts/CLAUDE.md). | 2026-10-01 (or the next provwasm-test-tube bump, whichever first) |

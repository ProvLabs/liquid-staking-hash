...
- **Wallet adapter:** a minimal interface (`connect`, `address`, `signAndBroadcast`) with implementations per supported wallet (`[DECIDE §14.1]`) plus a devnet direct-key mode that is compile-time excluded from production builds.
...
| `gas_price` | `1905nhash` **[VERIFY §14.3]** | Fee estimation basis. |
...
Every control is danger- or warning-styled and confirm-gated (§10.4). The page opens with a reminder that authority is the `x/group` policy: the console builds the message, the group process executes it **[DECIDE §14.6]** (direct execute when the connected account IS the policy executor; otherwise the console offers the encoded message JSON for a group proposal).
...
A minimal adapter interface: `connect() → address`, `disconnect()`, `signAndBroadcast(msgs, fee, memo) → txhash`. Implementations: Provenance-capable browser extension wallet(s) `[DECIDE §14.1]`, plus a devnet direct-key signer enabled only by the devnet build profile (§7) and visually marked (persistent warning chip "devnet key mode"). The console never handles mnemonics or keys outside that devnet mode; production signing happens entirely in the wallet extension.

### 10.2 Transaction lifecycle

1. **Build:** typed `ExecuteMsg` construction; funds attached only for `PayCommission`/`PayTip`.
2. **Simulate:** gas estimation against the LCD; fee = gas × `gas_price` with a 1.3 adjustment factor. Simulation failure surfaces the contract error *before* signing (most guard violations are caught here even if preflight missed them).
...
- **Self-contained delivery:** no third-party CDNs, fonts, or analytics; a strict CSP (`default-src 'self'; connect-src <lcd_url>`) so the only network peer is the configured node. Typography is system fonts only (§11.3) — nothing to load, no `font-src` exposure. No cookies; `localStorage`/IndexedDB hold only theme (Auto/Light/Dark), ledger, and wallet-connection hints.
...
1. **[DECIDE] Wallet support set.** Which Provenance-capable wallets ship in v1 (Figure/Provenance extension, Keplr with the Provenance chain config, Leap), and whether WalletConnect is in scope. Drives the §10.1 adapter implementations. *Recommendation: one extension wallet + the devnet key mode for v1; add others after.*
2. **[VERIFY] LCD surface against the deployed chain:** exact vault REST paths and response field names for vault config/NAV, pending swap-outs, and estimate-swap-out; principal-marker address derivation for the balance query; CORS posture of the program-operated node; `gas_price` for fee estimation. *Partially settled on devnet (2026-07-09): the vault route is `GET /vault/v1/vaults/{addr}` (not `provlabs.vault.v1`-prefixed) and returns `vault`, `principal` (marker address + coins, removing the derivation question), and `total_vault_value` directly; pending swap-outs at `.../pending_swap_outs`. Re-confirm on the test/mainnet build.*
...
5. **[DECIDE] Epoch-ledger backfill** from historical `RunEpoch` tx events (v1 vs v1.1). v1 without backfill is functionally complete but history-poor for new browsers.
6. **[DECIDE] Admin execution path under `x/group`:** direct execute (when the connected signer is the policy executor) vs "compose message JSON for a group proposal" mode, or both (§8.7). Depends on the admin group's operational setup.
7. **[DECIDE] Moniker fallback and identicon policy** for validators missing metadata; and whether to link validators to an external explorer profile.
8. **[DECIDE] Hosting and domain**, plus whether devnet/testnet builds are publicly reachable or team-only.
...
5. **Tx layer + wallet adapter:** devnet key mode first, extension wallet second; guard preflights; confirmation tiers; toast lifecycle.

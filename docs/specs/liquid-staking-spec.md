...
- **Invariant (verified continuously):** `receipt outstanding == receipt held by the principal marker == receipt bank supply == nhash out on chain (delegated + unbonding + pending deployment)`, exact to the base unit. The `ReceiptAccounting {}` query (§11.3) serves the invariant's legs from one consistent state read, so an external verifier reconciles it without stitching multiple queries at different heights.

All settlement legs execute **unpaused** — the vault refuses to settle assets while paused. Two operations are pause-gated: the reward deposit, and any repricing of the receipt, which the vault holds (vault v1.2.4 requires a paused vault to reprice a held asset, since a price step moves the share price). The write-down's markdown and restore are therefore each bracketed by a pause, with the extraction settling live between them; every bracket sits inside the same atomic epoch transaction (§9.9), so no user operation observes an intermediate state. The full deploy/return/burn/write-down cycle is exercised end to end by the repository's devnet drills and its multi-epoch simulation suite.
...
4. **Slash write-down** (only when needed): unbacked receipt is extracted via a zero-priced settlement under the contract's NAV authority — the 1:1 entry is marked for exactly the unbacked units inside a pause bracket, the extraction settles with the vault live, and the 1:1 entry is restored inside the closing bracket (step 5) — then burned, marking TVV down the same epoch the loss is detected.
5. **Atomic pause window** (when there is a deposit, a write-down, or both): `PauseVault` → the 1:1 NAV restore if this crank wrote down → `DepositPrincipalFunds` (the NAV step-up) if there is a surplus → `UnpauseVault`, all inside the same transaction. A crank never carries both, because the whole matured amount settles whenever liquid covers it.
...
### 11.1 State
- `config`: `{ admin, vault_address, underlying_denom, receipt_denom, max_delegations_per_run, aum_fee_bps (admin mirror of the vault fee), performance_threshold_bps, min_capture_interval_secs, max_concentration_multiple_bps / min_bonded_cap_bps / max_bonded_cap_bps (chain-restriction mirrors), concentration_safety_offset_bps, commission_bps, jail_unbond_delay_secs, redemption_margin_bps (redemption reserve over-coverage, `0..=1_000` bps rejected outside, default 50; absent pre-8.4a stored state deserializes to 50 — §14 D29) }` (bridge config — `bridge_address`, `bridge_enabled` — lives on the **vault**, not the staking contract). The epoch cadence is no longer a config value: `min_run_interval_secs` was **retired** in favor of the calendar-month rollover predicate (§9), which is derived from consensus block time and takes no parameter.
...
| `UpdateConfig { ...optional fields }` | admin | Update rates, thresholds, cadence, cap mirrors, cooldowns, and `redemption_margin_bps` (`0..=1_000`, rejected outside — never clamped). Only supplied fields change. |
...
record (wrong-artifact guard) and rejects a **downgrade** — the incoming
crate version must parse as semver and be `>=` the stored cw2 version
(same-version migration is permitted; an unparseable version in either
position is rejected). It then re-stamps the cw2 version and touches no other
state — the storage layout is shared across code versions, and a future
layout change must ship its transformation in this entry point. Standing rule
for future migrations: a migrate that transforms state must handle the
`EpochPhase::Releasing` continuation state (pending queues non-empty)
explicitly or require `Idle`; the v1 no-op is exempt only because it
transforms nothing.

### 11.3 Query endpoints
`Config {}`; `Validators {}` (every enrolled validator with its live assessment — uptime, jailed/tombstoned, commission accrued/paid/due and arrears, TIP, eligibility, concentration headroom — sorted by program priority, the reverse of the drain order; this single endpoint carries the priority, commission and tip views); `EpochStatus {}` (phase, halted, last run, `receipt_minted`, pending continuation queues — the §5.1 invariant view); `JailReports {}` (open reports with their purge-ready times); `EpochSnapshot {}` (the last epoch's value decomposition, §9.10); `Apr {}` (gross and net-of-drags APR in bps with the full breakdown, §9.10 / §17 R2); `ReceiptAccounting {}` (the §5.1 invariant's legs from ONE consistent state read — `receipt_minted`, bank `receipt_bank_supply`, program `staked`, in-flight `unbonding`, earmarked `pending_deployment`, and the saturating `matured_unsettled` residual; permissionless, read-only, bounded by the validator ceiling).

### 11.4 How the contract drives the vault
The contract emits vault and exchange module messages under its asset-manager (and NAV) authority as protobuf `CosmosMsg::Any` values, and reads vault, staking, slashing, distribution and marker state via the whitelisted query set. All message routing and authority gating is verified against a live chain by the repository's devnet drills: settlements (`MsgCreatePayment` + `AcceptAsset`, the approval carrying the full payment terms), the paused `DepositPrincipalFunds`, `ExpeditePendingSwapOut`, `PauseVault`/`UnpauseVault`, `UpdateVaultNAV` (write-down sandwich, each repricing pause-bracketed), and the marker mint/transfer/burn cycle.
...
- **Asset-manager and NAV authority are powerful.** The contract can settle assets against the vault, deposit into principal, pause and unpause it, and (as the vault's NAV authority) reprice the receipt — the repricing power exists solely for the slash write-down sandwich (§9.9), which marks the entry, extracts exactly the unbacked units at zero price, and restores 1:1 within one atomic transaction, each repricing inside a pause bracket the same transaction closes. Mitigations: deterministic, auditable engine logic; the exact-price settlement guardrail (a settlement cannot trade off the recorded NAV); settlement approvals that carry the full payment terms, so the vault settles only the exact deal the contract signed; the receipt-accounting invariant (§5.1), continuously machine-checked; pause and halt controls; governance-held admin; and the unused interest-rate lever (§5). The NAV-authority grant is marginal on top of the contract's existing receipt mint/burn powers and is explicitly in scope for the audit.
...
- **Contract code replacement** (`MsgMigrateContract`, plus `MsgUpdateAdmin`/`MsgClearAdmin` over the admin itself) → the **admin group policy**, set at instantiate (`--admin`, the bootstrap's `WASM_ADMIN` hook) so a code migration requires the same group threshold as any admin action. It is the admin policy and never the ops policy because code migration is fund administration at its most extreme — it belongs with pause/halt under the (eventually tighter, 8.5) admin threshold, not with parameter tuning. This authority subsumes every other power the contract holds — it can install arbitrary code carrying the asset-manager, mint/burn, and NAV authorities — so it is never left with a single key after bring-up. The §11.2 migrate entry point is its only in-contract surface: an in-contract cw2 gate rejects downgrades and foreign contract names; the v1 migrate performs no state transformation; a future state-transforming migration must handle the `Releasing` phase explicitly or require `Idle` (§11.2).
...
**As built:** the contract accepts a single `admin` authority address, deployed as an `x/group` policy account so membership is rotatable and approvals are threshold-gated without redeployment. The `admin`/`ops` split is resolved at the **bootstrap level** (D25, 2026-08-14): every non-devnet deployment creates both policies before instantiate and the deployment record names which holds `Config.admin` and which holds the wasmd migrate admin; the contract keeps a single `Config.admin` and no contract-side split is performed — deliberately, since `ExecuteMsg` has no admin-rotation variant and a second in-contract authority would widen the audit surface for no threshold the two-policy bootstrap does not already provide. Membership and thresholds for each policy are launch-operations parameters, not design questions. The 1..n policy-set rule below stands verbatim: with two policies the floor on every non-devnet deployment is **two**, and nothing may resolve "the policy" from the set.
...
>   convenience.
> - **The policy set is 1..n, and nothing may assume 1.** Every consumer
>   discovers the set (`Config.admin` → policy → group → all policies on that
>   group). With the D25 bootstrap-level split (2026-08-14) the floor on every
>   non-devnet deployment is two policies; the devnet group carries two by
>   design, so discovery is exercised by data. `ExecuteMsg` still has no
>   admin-rotation variant and `InstantiateMsg.admin` is set once, so any
>   future contract-side authority change remains a spec-level event under the
>   enumerated-trust-surfaces rule, not a config change.
...
4. **Redemption liquidity (§8)** — 60-day standard delay + marker-gated expedite, drilled: matured and expedited payouts at stepped-up NAV, and the unfunded-maturity failure mode observed as a clean share refund (no pause, no loss). The reserve margin's calibration finding is closed by D29 (item 15, 2026-08-14): `redemption_margin_bps` is governance-tunable (`0..=1_000`, default 50), with both bound edges pinned by simulation boundary scenarios; sizing the launch value stays a §14 launch-checklist row (CO-37).
...
9. **Slash recognition (§9.9)** — write-down the epoch of detection via the NAV-authority sandwich; drilled: TVV marked down by exactly the unbacked amount, invariant restored to equality. **Amended 2026-08-13 (vault v1.2.4):** the sandwich's two repricings are now each pause-bracketed — the vault requires a paused vault to reprice an asset it holds and a live vault to settle one — with the closing bracket also carrying the reward deposit when a crank has one. The brackets open and close inside the single epoch transaction, so the §9.9 atomicity argument is unchanged, and the emitted sequence is machine-checked (`epoch.rs` sequence_tests: settlements live, off-par repricings paused). Re-drill of the TVV identity against the pinned release is the open confirmation, tracked in `contracts/IMPLEMENTATION-STATUS.md` §3.
10. **Governance (§12.1)** — single admin `x/group` policy as built; the admin/ops policy split is resolved at the bootstrap level by D25 (§12.1 as-built, 2026-08-14). **Amended 2026-07-29 (App PR 7.1 commit A):** the policy topology was previously described but never exercised — the devnet ran a plain-account `Config.admin`, so no privileged call had ever passed through a proposal. A group + threshold policy is now bootstrapped before deploy (`infra/devnet/bootstrap/nvhash-group-bootstrap.sh`; it must run first because `ExecuteMsg` has no admin-rotation variant) and the full lifecycle is drilled (`contracts/drills/gov-drill.sh`, 30 assertions, result dated in `contracts/IMPLEMENTATION-STATUS.md`). The drill also established that the §12.1 "full proposal/vote audit trail" is **not** a chain property: a successfully executed proposal is pruned in its own transaction and votes are deleted at voting-period end, so the trail lives in transaction history and the App's mirror. One state remains unproduced — `PROPOSAL_STATUS_ABORTED` was not reachable on the drilled build (a mid-vote group change did not abort an open proposal), recorded in the fixture manifest rather than assumed away.
...
13. **Vault module version (§5.1/§9.9/§11.4)** — resolved 2026-08-13 by pinning **ProvLabs/vault v1.2.4** ([plan](../plans/2026-08-13-m8.0-contracts-vault-1.2.4-upgrade.md)), superseding the 2026-07-08/09 reconciliation against vault `main` and retiring the feature-probe compatibility rule: the 1.2.x line is the first to ship `AcceptAsset` upstream, so "compatible vault" is now a version. Four release behaviors bear on this design. (a) A settlement approval carries the **full payment terms** and the vault settles only on an exact match, so the contract builds both legs of a settlement from one value. (b) Repricing an asset the vault **holds** requires a paused vault, which is what bracketed the write-down sandwich (item 9). (c) An outbound settlement that drains a denom **removes** its internal NAV entry, and a settlement against an unpriced denom is rejected — so the contract restates the receipt's 1:1 entry immediately before **each** par settlement leg, not once per crank: on a full unwind the return leg is itself the settlement that drains the entry, and the deploy leg later in the same transaction would otherwise be rejected. The restate is legal on a live vault in every reachable state (the entry is either absent, meaning the vault holds none, or already at par). The write-down extraction is the one settlement not preceded by a restate, since it must execute at the marked-down price. (d) Share-NAV publication is uint64-safe upstream, closing that tail. The launch chain must ship v1.2.4 or later; drill re-verification against the pinned build and the App fixture re-capture are the open confirmations (`contracts/IMPLEMENTATION-STATUS.md` §3).

14. **Contract migratability (D21, §11.2/§12.1)** — resolved 2026-08-14 ([plan](../plans/2026-08-14-app-m8.4a-migration-mode.md)): the migrate entry point ships the cw2 name + semver-downgrade gate (unit quadrants passing); the wasmd contract admin is set at instantiate to the **admin** group policy (bootstrap `WASM_ADMIN` hook, defaulting to `CONTRACT_ADMIN`), so migration carries the same group threshold as pause/halt. The live verification drill (`contracts/drills/migrate-drill.sh` — unauthorized rejection under a keyless policy admin, group-executed migrate, same-code-id store probe, byte-for-byte state-dump diff across a version bump, downgrade accepted-by-group but failed-in-execution) is **authored but not yet run**; its [VERIFY] closures stay open in `contracts/IMPLEMENTATION-STATUS.md` until the first fresh group-first devnet run, and this row records the drill date and node image when it lands.

15. **Redemption margin configurability + receipt-accounting query (D29, §8/§5.1/§11)** — resolved 2026-08-14: `redemption_margin_bps` joins `config` (`0..=1_000` rejected outside, default 50; pre-existing stored state deserializes to 50 by serde function default, regression-pinned), closing item 4's 2026-07-09 calibration finding's recommendation; simulation boundary scenarios pin both edges (margin-zero must produce refunds — the modeled safe outcome; margin-max must still pay redemptions with every §4.2-family invariant intact). `ReceiptAccounting {}` (§11.3) serves the §5.1 invariant's legs from one consistent state read. Launch-time sizing of the margin value remains open as the checklist row below / CO-37.
...
- ProvLabs Vault module (source of vault facts), **pinned at tag `v1.2.4`** — the version this program targets; read the tag, not `main`: https://github.com/ProvLabs/vault/tree/v1.2.4 — concepts: [`spec/01_concepts.md`](https://github.com/ProvLabs/vault/blob/v1.2.4/spec/01_concepts.md), messages: [`spec/03_messages.md`](https://github.com/ProvLabs/vault/blob/v1.2.4/spec/03_messages.md), block hooks: [`spec/06_blocker.md`](https://github.com/ProvLabs/vault/blob/v1.2.4/spec/06_blocker.md), protos: [`proto/provlabs/vault/v1`](https://github.com/ProvLabs/vault/tree/v1.2.4/proto/provlabs/vault/v1)

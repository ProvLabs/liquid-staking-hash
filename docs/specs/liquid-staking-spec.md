...
Runs **monthly**, computed and executed **entirely in the contract** (no off-chain planner). Objective: within a single epoch session, drive **every eligible validator to the same uniform slot-sized delegation**. TIP/uptime priority (§10.2) governs only drain order (which validators are unbonded first for redemption liquidity) and tie-breaks.
...
- **The monthly cadence clears the locks.** Redelegations lock for the ~3-week unbonding period, which is **shorter than the monthly epoch**, so the previous epoch's redelegation entries have matured before the next epoch begins — each epoch starts with clean `MaxEntries` capacity (the cap is per `(delegator, validator)` pair; one rebalance creates ≤1 entry per pair, far under the limit).
...
### 11.1 State
- `config`: `{ admin, vault_address, underlying_denom, receipt_denom, min_run_interval_secs (≈ monthly; calendar-month alignment pending — §14 note), max_delegations_per_run, aum_fee_bps (admin mirror of the vault fee), performance_threshold_bps, min_capture_interval_secs, max_concentration_multiple_bps / min_bonded_cap_bps / max_bonded_cap_bps (chain-restriction mirrors), concentration_safety_offset_bps, commission_bps, jail_unbond_delay_secs }` (bridge config — `bridge_address`, `bridge_enabled` — lives on the **vault**, not the staking contract)
...
**Pending post-1.0 design change — calendar-month epoch alignment (opened 2026-07-15).** The App spec's §14.12 decision aligns epochs to **calendar-month boundaries computed from block time** (`env.block.time`, the consensus-agreed BFT timestamp — the only valid deterministic clock in contract execution; it is Unix/UTC-based, but its authority is consensus agreement, never any node's wall clock or an external UTC source) and displays them as such. The contract as built does **not** do this: `RunEpoch` is gated on `min_run_interval_secs (≈ monthly)`, a fixed-duration minimum interval, not a calendar boundary. The change **retires** that interval gate in favor of month-rollover eligibility (`civil_month(block time) > civil_month(last_run)`). Note the gate is an eligibility floor, not a trigger — the epoch still ends only when a permissionless caller cranks, so epoch durations were always variable; the change makes the earliest-valid boundary calendar-deterministic and caller-independent. It is a real contract behavior change: run spacing becomes nominally 28–31 days but a late crank can compress the following gap below the unbonding period (safe by the §9.3 plan-time deferral guards, to be asserted in simulation, not assumed); the `withdrawal_delay_seconds` ceiling must be re-pinned; and the simulation domain and per-epoch invariant assertions must be extended in the same change per `SECURITY.md`. It is **scheduled, not implemented** — see [`docs/plans/2026-07-15-calendar-month-epoch-alignment.md`](../plans/2026-07-15-calendar-month-epoch-alignment.md) and the `IMPLEMENTATION-STATUS.md` §2 entry. Until it lands, the design text below and in §9 describes the fixed-interval cadence as built; the App's "calendar month" copy matches the contract only once this change ships.

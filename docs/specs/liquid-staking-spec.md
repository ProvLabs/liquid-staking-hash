...
### 11.1 State
- `config`: `{ admin, vault_address, underlying_denom, receipt_denom, min_run_interval_secs (≈ monthly), max_delegations_per_run, aum_fee_bps (admin mirror of the vault fee), performance_threshold_bps, min_capture_interval_secs, max_concentration_multiple_bps / min_bonded_cap_bps / max_bonded_cap_bps (chain-restriction mirrors), concentration_safety_offset_bps, commission_bps, jail_unbond_delay_secs }` (bridge config — `bridge_address`, `bridge_enabled` — lives on the **vault**, not the staking contract)

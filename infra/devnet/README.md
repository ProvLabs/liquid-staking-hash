...
The `ghcr.io/provlabs/vault-dev-node:latest` Docker image containing the
settlement-era vault module (`AcceptAsset`) — pulled automatically by the
lifecycle script when absent (authenticate with `docker login ghcr.io` if the
pull is denied). No tagged vault release ships `AcceptAsset` yet;
compatibility is by feature probe — see the vault version gating item in
`contracts/IMPLEMENTATION-STATUS.md`. To use a locally built image instead
(`make docker-build-dev` from a Provenance checkout wired to pre-release
vault `main`), set `IMAGE` to its tag. Everything else is handled by the
lifecycle script; genesis is generated by the image itself and patched for

...
#
# DEVNET ONLY (SECURITY.md "Development environment"): these keys are
# throwaway test material by definition. The chain-id guard below refuses any
# chain outside the dev family, and nothing is written to the repo — the
# keyring lives in the gitignored infra/devnet/state/. check:bundle's sentinel
# scan (standing) keeps the signer out of any shipped bundle.
set -euo pipefail

...
# The wasmd contract admin (MsgMigrateContract authority) — distinct from
# InstantiateMsg.admin, fixed at instantiate. Defaults to CONTRACT_ADMIN so
# the governed bootstrap puts both authorities on the admin policy (D21).
# No no-admin path: renouncing upgradability is MsgClearAdmin later.

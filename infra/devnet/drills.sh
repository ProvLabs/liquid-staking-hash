...
#
# This script sequences the failures on the host; the specs
# (apps/web/e2e-live/drills/) only observe HTTP. Every wait is bounded and
# fails the phase on expiry. A red drill is a finding — never widen a
# tolerance to make one pass.
#
# DEVNET ONLY (SECURITY.md): the chain-id guard refuses any chain outside the
# chain-dev family.
set -euo pipefail

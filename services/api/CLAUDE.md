...
- **Envelope contract** (`test/envelope-contract.test.ts`): registry-driven —
  ...
  cannot slip past the harness. The suite also pins the PR 4.2/4.3-frozen
  scaffold shapes: `/metrics` (exact `ProgramMetrics` field set, all null),
  `/epochs`, and `/validators` (empty, pagination-bounded), with row types
  in `@nvhash/api-types` that PR 3.1 implements against.
